...
function affiche_points ($user_id, $gr_champ, $idconnect)
{
 // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $result=$idconnect->query("SELECT points 
						    FROM phpbg_clmnt_pronos, phpbg_membres 
						    WHERE phpbg_membres.id=phpbg_clmnt_pronos.id_membre 
						    and phpbg_membres.id_prono='$user_id' and type='general' 
...
function affiche_clmnt_general ($user_id, $gr_champ, $idconnect)
{
  ...
  $result=$idconnect->query(" SELECT id_prono 
							  ...
							  AND type='general' 
							  AND id_champ='$gr_champ' 
							  ORDER by points desc, participation desc, phpbg_membres.pseudo");
  //$result=mysql_query($query) or die (mysql_error());
  $i = "1";
  while ($row=mysqli_fetch_array($result))
  {
...
$result=$idconnect->query("SELECT phpbg_clubs.nom, CLEXT.nom, phpbg_matchs.buts_dom, phpbg_matchs.buts_ext, phpbg_matchs.id, phpbg_matchs.date_reelle, phpbg_journees.numero
FROM phpbg_clubs, phpbg_clubs as CLEXT, phpbg_matchs, phpbg_journees, phpbg_equipes, phpbg_equipes as EXT, phpbg_gr_championnats
...
AND phpbg_journees.id_champ=phpbg_gr_championnats.id_champ
AND phpbg_gr_championnats.id='$gr_champ'
AND phpbg_matchs.buts_dom is null
AND phpbg_matchs.buts_ext is null
AND phpbg_clubs.nom!='exempte'
AND CLEXT.nom!='exempte'
ORDER by phpbg_matchs.date_reelle, phpbg_clubs.nom
LIMIT 0, 10");
...
$result2=$idconnect->query( "SELECT pronostic 
							FROM phpbg_pronostics, phpbg_gr_championnats 
							WHERE phpbg_pronostics.id_match='$row[4]' 
							AND phpbg_gr_championnats.id='$gr_champ' 
...
if ($nb_pronos == "0") {$prono="0";}
{
  ...
  $resultats2=$idconnect->query("SELECT tps_avant_prono FROM phpbg_gr_championnats WHERE id='$gr_champ'");
...
{
   mysqli_query($idconnect, ("DELETE FROM phpbg_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_en_cours'"));

   $result=$idconnect->query("SELECT id_membre, pseudo, sum(points) as total, sum(participation) as participations
   FROM phpbg_membres, phpbg_pronostics, phpbg_matchs, phpbg_gr_championnats
   WHERE phpbg_pronostics.id_champ=phpbg_gr_championnats.id
   AND phpbg_gr_championnats.id='$gr_champ'
   AND id_membre=phpbg_membres.id
   AND phpbg_matchs.id=id_match
 ...
 //  $result=mysql_query ($query) or die ("probleme " .mysql_error());
   while ($row=mysqli_fetch_array($result))
         ...
         mysqli_query($idconnect, ("INSERT INTO phpbg_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_en_cours')"));
         }
}          

if ($type=="mensuel_30_jours")
{
   mysqli_query($idconnect, ("DELETE FROM phpbg_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_30_jours'"));
   ...
   //$result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       mysqli_query($idconnect, ("INSERT INTO phpbg_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_30_jours')"));
       }
}

if ($type=="hebdo")
{
   mysqli_query($idconnect, ("DELETE FROM phpbg_clmnt_pronos WHERE id_champ='$gr_champ' AND type='hebdo'"));
  ...
  // $result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       mysqli_query($idconnect, ("INSERT INTO phpbg_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) 
								  VALUES ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'hebdo')"));
       }
}
if (isset($_REQUEST['complet'])) {$complet=$_REQUEST['complet'];} else {$complet='';}

$result=$idconnect->query("SELECT pseudo, points, participation FROM phpbg_clmnt_pronos
...
function pseudo_admin ($gr_champ, $idconnect)
{
 $resultat=$idconnect->query("SELECT pseudo 
							  FROM phpbg_membres, phpbg_gr_championnats 
							  WHERE phpbg_gr_championnats.id_master=phpbg_membres.id 
							  AND phpbg_gr_championnats.id='$gr_champ'");
...
function champ_prono ($gr_champ, $idconnect)
{
 ...
 $resultat=$idconnect->query("SELECT DISTINCT id, nom FROM phpbg_gr_championnats WHERE  phpbg_gr_championnats.activ_prono='1' ORDER by id");
