...
function aff_journee($champ, $numero, $legende, $proba, $fiches_clubs, $id_equipe_fetiche, $idconnect)
  ...
  $color=0;
        $result=$idconnect->query("SELECT cldom.nom as cldom, clext.nom as clext, phpbg_matchs.buts_dom, phpbg_matchs.buts_ext,
...
//mise en forme tableau oppositions
        echo "<table align=\"center\" class=\"tablephpbg2\"  width=\"100%\" cellspacing=\"1\" cellpadding=\"1\"><tr><td><table cellspacing=\"0\" align=\"center\" width=\"100%\" cellpadding=\"1\">\n";
        $x=1;
        $minute = 0;
        $heure = 0;
        $jour = 0;
        ...
        $annee = 0;
             ...
             $extproba= $row[3];
                ...
                {
                $result2=$idconnect->query("SELECT DOMBUTSPOUR, DOMG, DOMN, DOMP, DOMBUTSCONTRE FROM phpbg_clmnt_cache WHERE ID_EQUIPE='$row[eqdom]'");
                ...
                $result2=$idconnect->query("SELECT EXTG, EXTN, EXTP, EXTBUTSCONTRE, EXTBUTSPOUR FROM phpbg_clmnt_cache WHERE ID_EQUIPE='$row[eqext]'");
               // $result2=mysql_query($query2);
...
/************* mise en commentaire suite passage php7 ******************
  $requete=$idconnect->query("SELECT phpbg_clubs.nom, CLEXT.nom, phpbg_matchs.buts_dom, phpbg_matchs.buts_ext,
            phpbg_matchs.id, phpbg_matchs.date_reelle
  ...
  {   
   ...
   $row[1]= stripslashes($row[1]);
    ...
    if ($row[0]=='exempte') {echo "<tr bgcolor=\"$bgcolor1\" class=\"trphpbg\"><td colspan=\"7\">".ADMIN_RESULTS_1." : $row[1]</td></tr>";}
    if ($row[1]=='exempte') {echo "<tr bgcolor=\"$bgcolor1\" class=\"trphpbg\"><td colspan=\"7\" >".ADMIN_RESULTS_1." : $row[0]</td></tr>";}
  }
 */
...
{
  $nb_requete=0;
  if($cache=="1"){ mysqli_query("DELETE FROM phpbg_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysqli_error());}
  else{ mysqli_query("DELETE FROM phpbg_clmnt") or die (mysqli_error());}
          $nb_requete++;
...
$result=$idconnect->query("SELECT pts_victoire, pts_nul, pts_defaite FROM phpbg_parametres WHERE id_champ='$champ'");
$nb_requete++;
...
$query=$idconnect->query("SELECT phpbg_clubs.nom FROM phpbg_clubs, phpbg_equipes, phpbg_championnats
...
//mysql_query("LOCK TABLE phpbg_clmnt WRITE, phpbg_equipes WRITE, phpbg_clubs WRITE") or die (mysql_error());
    ...
    {
    $result=$idconnect->query("SELECT phpbg_clubs.nom, phpbg_equipes.id 
            FROM phpbg_clubs, phpbg_equipes, phpbg_championnats
            WHERE phpbg_equipes.id_champ=phpbg_championnats.id
            AND phpbg_championnats.id='$champ'
            AND phpbg_equipes.id_club=phpbg_clubs.id");
...
if($cache=="1"){$requete="DELETE FROM phpbg_clmnt_cache WHERE nom='exempte'" or die (mysql_error());}
else{$requete="DELETE FROM phpbg_clmnt WHERE nom='exempte'" or die (mysql_error());}
...
function clmntmini_barre($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini,  $champmini, $requetemini, $nb_dessusmini, $nb_dessousmini, $lienmini, $phpbgEAGUE_RACINE, $id_equipe_fetiche)
{
echo "<table class=tablephpbg2 align=\"center\" cellspacing=\"0\" width=\"200\"><tr class=trphpbg3><th colspan=10>".$legendemini;
echo "<tr class=trphpbg3>";
echo "<th align=center>".CLMNT_POSITION."";
...
function clmntred($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini, $champmini, $requetemini, $lienmini, $phpbgEAGUE_RACINE, $id_equipe_fetiche)
{
echo "<table class=tablephpbg2 align=\"center\" cellspacing=\"0\"  width=\"200\"><tr class=trphpbg3><th colspan=10>".$legendemini;
echo "<tr class=trphpbg3>
               ...
               echo "<td>";
                    ...
                    if ($x==0)
                    {
                     ...
                     if ($lienmini=='non'){echo "$row[$x]";}
                      ...
                      {
                        echo "<a href=\"".$phpbgEAGUE_RACINE."consult/detaileq.php?champ=$champmini&amp;id_equipe=".$row['ID_EQUIPE']."\" target=\"_parent\">$row[$x]</a>";
                      }
                     if ($row['ID_EQUIPE']==$id_equipe_fetiche){echo "</b>";} 
                     }
...
function clmnt_barrered($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini,  $champmini, $requetemini, $lienmini, $phpbgEAGUE_RACINE, $id_equipe_fetiche)
{
