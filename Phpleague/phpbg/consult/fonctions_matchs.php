...
function nb_journees($id_champ, $idconnect)
         {
        global $nb_journees;
$result=$idconnect->query("SELECT id FROM phpbg_equipes WHERE id_champ='$id_champ'");
...
function nb_equipes($id_champ, $idconnect)
 ...
 $res=$idconnect->query("SELECT id 
        ...
        // $query="SELECT id FROM phpbg_equipes WHERE id_champ='$id_champ'";
        // $result=mysql_query($query);
...
function aff_journee($champ, $bdd)
{
 ...
 $res3=$bdd->query("SELECT phpbg_matchs.id_journee
					FROM phpbg_journees, phpbg_matchs 
					WHERE phpbg_journees.id = phpbg_matchs.id_journee 
					AND phpbg_matchs.buts_dom is not NULL 
					AND phpbg_matchs.buts_ext is not NULL
					AND phpbg_journees.id_champ = '$champ'
					ORDER BY phpbg_matchs.id_journee asc");
...
//  echo "journee : ".$journee;echo "</br>";
 ...
 //echo $numero;
 // echo "</br>";
 $result=$bdd->query("SELECT cldom.nom as cldom, clext.nom as clext, phpbg_matchs.buts_dom, phpbg_matchs.buts_ext,
                        phpbg_journees.date_prevue, cldom.id as cliddom, clext.id as clidext, date_reelle,
                        dom.id as eqdom, ext.id as eqext, phpbg_matchs.id as id_match
                FROM phpbg_equipes as dom, phpbg_equipes as ext, phpbg_matchs, phpbg_journees,
                     phpbg_clubs as cldom, phpbg_clubs as clext
  ...
  echo "<br />";
  echo "<table align=\"center\" class=\"tablephpbg2\"  width=\"90%\" cellspacing=\"1\" cellpadding=\"1\"><tr><td><table cellspacing=\"0\" align=\"center\" width=\"100%\" cellpadding=\"1\">\n";
  $x=1;
  ...
  while ($row=$result->fetch())
	{
     ...
     if (($color%2)==0) {$classe="ligne1";} else {$classe="ligne2";}
                ...
                //echo "<tr bgcolor=\"$bgcolor\" class=\"trphpbg\">";
...
function aff_journeeSupp($champ, $bdd)
{
 ...
 $result=$bdd->query("SELECT cldom.nom as cldom, clext.nom as clext, phpbg_matchs.buts_dom, phpbg_matchs.buts_ext,
                ...
                WHERE phpbg_matchs.id_equipe_dom=dom.id
                        AND phpbg_matchs.id_equipe_ext=ext.id
                        AND phpbg_journees.id_champ='$champ'
                        AND phpbg_journees.numero='$journeeSupp'
                        AND dom.id_club=cldom.id
                        AND ext.id_club=clext.id
                        AND phpbg_matchs.id_journee=phpbg_journees.id
                        AND cldom.nom!='exempte'
...
function classementDetaille($champ, $bdd)
{
 ...
 while ($row = $res->fetch())
  ...
  echo "<table class=\"tablephpbg2\" cellspacing=\"0\" width=\"80%\"><tr class=\"trphpbg3\"><th colspan=\"13\">".$legende."</th></tr>\n";
  echo "<tr class=\"trphpbg3\">
 ...
 // {
	...
	$res=$bdd->query("SELECT NOM, POINTS, JOUES, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, PTS_ADMIN
					  FROM phpbg_clmnt_cache 
					  WHERE ID_CHAMP='$champ' 
					  ORDER BY POINTS DESC, PEN DESC, DIFF DESC, BUTSPOUR DESC , BUTSCONTRE 
					  ASC, NOM");

	//echo "<table class=\"tablephpbg2\" align=\"center\" cellspacing=\"0\" width=\"80%\"><tr class=\"trphpbg3\"><th colspan=\"11\">".$legende."</th></tr>\n";
	//echo "<tr class=\"trphpbg3\>";
...
{
 $resultat=$bdd->query("
		  SELECT phpbg_clubs.nom FROM phpbg_clubs, phpbg_equipes 
		  WHERE phpbg_equipes.id='$id_equipe' 
		  AND phpbg_equipes.id_club=phpbg_clubs.id");
...
echo "<table class=\"tablephpbg2\" align=\"center\" cellspacing=\"0\"  width=\"90%\">";

$resultat=$bdd->query("
		SELECT phpbg_journees.numero, cldom.nom, clext.nom, phpbg_matchs.buts_dom, phpbg_matchs.buts_ext, phpbg_matchs.date_reelle, phpbg_matchs.id
        FROM phpbg_equipes as dom, phpbg_equipes as ext, phpbg_matchs, phpbg_journees, phpbg_clubs as cldom  , phpbg_clubs as clext
        WHERE phpbg_matchs.id_equipe_dom=dom.id
        AND phpbg_matchs.id_equipe_ext=ext.id
        AND (phpbg_matchs.id_equipe_ext='$id_equipe'
        OR phpbg_matchs.id_equipe_dom='$id_equipe')
        AND phpbg_journees.id_champ='$champ'
        AND dom.id_club=cldom.id
        AND ext.id_club=clext.id
        AND phpbg_matchs.id_journee=phpbg_journees.id
...
echo "<br />";echo "<br />";
	if ($smart == false)
	echo "<tr class=\"trphpbg3\"><td align=\"center\"><font color=\"#FFFF00\"><b>".JOURNEE."</b></font></td><td width=\"5% align=\"center\"></td><td width=\"30%\" ><font color=\"#FFFF00\"><b>".DATE."</b></font></td><td colspan=\"5\ align=\"center\"><font color=\"#FFFF00\"><b>Oppositions</b></font></td></tr>";
    else
	echo"<tr class=\"trphpbg3\"><td align=\"center\"><font color=\"#FFFF00\"><b>".JOURNEE."</b></font></td><td width=\"5% align=\"center\"></td><td colspan=\"5\ align=\"center\"><font color=\"#FFFF00\"><b>Oppositions</b></font></td></tr>";
...
{	
 ...
 $resultats2=$bdd->query("SELECT phpbg_clubs.nom, CLEXT.nom, phpbg_matchs.buts_dom, phpbg_matchs.buts_ext, phpbg_matchs.id, phpbg_matchs.date_reelle
 ...
 $result=$bdd->query("SELECT phpbg_journees.numero, phpbg_journees.date_prevue, cldom.nom, clext.nom, phpbg_matchs.buts_dom, phpbg_matchs.buts_ext, dom.id, ext.id
 ...
 while ($row=$result->fetch())
 {
  ...
  { 
   ...
   {   
    ...
    ?>    
	<table class="tablephpbg2" cellspacing="0" width="290" align="center">
	  <tr class="trphpbg3"><td colspan="3" align="center"><b><font color="#FFFF00">
...
function db_clmnt($champ, $debut, $fin, $cache, $idconnect)
{
  ...
  if($cache=="1"){ 
  mysqli_query("DELETE FROM phpbg_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysqli_error());
  }
  else{ 
  mysqli_query("DELETE FROM phpbg_clmnt") or die (mysqli_error());}
          $nb_requete++;
...
$resultats=$idconnect->query=("SELECT pts_victoire, pts_nul, pts_defaite FROM phpbg_parametres WHERE id_champ='$champ'");
$nb_requete++;
...
// victoires domicile
$resultats=$idconnect->query("SELECT dom.id, count(dom.id), phpbg_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phpbg_equipes as dom, phpbg_clubs, phpbg_matchs, phpbg_journees, phpbg_championnats
WHERE dom.id_champ='$champ'
      AND dom.id_club=phpbg_clubs.id
      AND dom.id=phpbg_matchs.id_equipe_dom
      AND buts_dom > buts_ext
      AND phpbg_championnats.id=phpbg_journees.id_champ
      AND phpbg_journees.id=phpbg_matchs.id_journee
      AND phpbg_journees.numero>='$debut'
      AND phpbg_journees.numero<='$fin'
      GROUP by phpbg_clubs.nom ");
      $nb_requete++;
...
// Defaites domicile
$dom=$idconnect->query("SELECT dom.id, count(dom.id), phpbg_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phpbg_equipes as dom, phpbg_clubs, phpbg_matchs, phpbg_journees, phpbg_championnats
WHERE dom.id_champ='$champ'
      AND dom.id_club=phpbg_clubs.id
      AND dom.id=phpbg_matchs.id_equipe_dom
      AND buts_dom < buts_ext
...
$result=$idconnect->query("SELECT phpbg_clubs.nom FROM phpbg_clubs, phpbg_equipes, phpbg_championnats
WHERE phpbg_equipes.id_champ=phpbg_championnats.id
...
// victoires exterieur
$dom=$idconnect->query("SELECT ext.id, count(ext.id), phpbg_clubs.nom, sum(buts_ext), sum(buts_dom) FROM phpbg_equipes as ext, phpbg_clubs, phpbg_matchs, phpbg_journees, phpbg_championnats
WHERE ext.id_champ='$champ'
      AND ext.id_club=phpbg_clubs.id
      AND ext.id=phpbg_matchs.id_equipe_ext
...
$result=$idconnect->query("SELECT phpbg_clubs.nom, phpbg_equipes.penalite, phpbg_equipes.id
        FROM phpbg_clubs, phpbg_equipes, phpbg_championnats
        WHERE phpbg_equipes.id_champ=phpbg_championnats.id
        AND phpbg_championnats.id='$champ'
        AND phpbg_equipes.id_club=phpbg_clubs.id");
...
//mysql_query("LOCK TABLE phpbg_clmnt WRITE, phpbg_equipes WRITE, phpbg_clubs WRITE") or die (mysql_error());

    if (mysqli_num_rows($result)==0)
    {
    $result="SELECT phpbg_clubs.nom, phpbg_equipes.id 
            FROM phpbg_clubs, phpbg_equipes, phpbg_championnats
            WHERE phpbg_equipes.id_champ=phpbg_championnats.id
            AND phpbg_championnats.id='$champ'
            AND phpbg_equipes.id_club=phpbg_clubs.id";
   ...
   //$result=mysqli_query($query) or die (mysqli_error());
    ...
    if($cache=="1"){$question.="phpbg_clmnt_cache ";}
    else{$question.="phpbg_clmnt ";}
...
if($cache=="1")
	{
	mysqli_query($idconnect, ("DELETE FROM phpbg_clmnt_cache WHERE nom='exempte'"));
	}
else
	{
	mysqli_query($idconnect, ("DELETE FROM phpbg_clmnt WHERE nom='exempte'"));
	}
...
{
echo "<table class=\"tablephpbg2\" align=\"center\" cellspacing=\"0\" width=\"80%\" bgcolor=\"#FFFFFF\"><tr class=\"trphpbg3\"><th colspan=\"11\">".$legende."<br /></th></tr>\n";
echo "<tr class=\"trphpbg3\"><th>".CLMNT_POSITION."</th><th align=\"left\">".MENU_NOM."</th><th align=\"left\">".TEAM."</th><th align=\"left\">".ADMIN_JOUEURS_2."</th></tr>\n";
...
//               statistiques, amélioration de la gestion des buteurs pour phpbgeague.          //
// Auteur      : Alexis MANGIN                                                                 //
// Email       : Alexis@univert.org                                                            //
// Url         : http://www.univert.org                                                        //
// Démo        : http://phpbgeague.univert.org/demo.php                                         //
// Description : Edition, gestion, fiches phpbg_clubs, statistiques, mini-classement...         //
...
//{
//$query="SELECT phpbg_donnee.id, phpbg_donnee.nom, id_rens, id_clubs, phpbg_rens.id, phpbg_rens.nom, phpbg_rens.id_classe, phpbg_clubs.id, etat, phpbg_donnee.url, phpbg_rens.url
//FROM phpbg_donnee, phpbg_rens, phpbg_clubs
//WHERE id_clubs='$id_clubs'
//      AND id_clubs=phpbg_clubs.id
//      AND id_rens=phpbg_rens.id
...
function clmntmini($legendemini, $typemini, $accessionmini, $barragemini, $relegationmini,  $champmini, $requetemini, $nb_dessusmini, $nb_dessousmini, $lienmini, $phpbgEAGUE_RACINE, $id_equipe_fetiche)
{

echo "<table class=tablephpbg2 align=\"center\" cellspacing=\"0\" width=\"200\"><tr class=trphpbg3><th colspan=10>".$legendemini;
echo "<tr class=trphpbg3>";
        ...
        $x=0;
               ...
               echo "<td align=\"left\">";
                    ...
                    if ($x==0)
                    {
                     ...
                     else {echo "<a href=\"".$phpbgEAGUE_RACINE."consult/detaileq.php?champ=$champmini&amp;id_equipe=".$row['ID_EQUIPE']."\" target=\"_parent\">$row[$x]</a>";}
                     if ($row['ID_EQUIPE']==$id_equipe_fetiche){echo "</b>";}
                     }
...
{
echo "<table class=\"tablephpbg2\" align=\"center\" cellspacing=\"0\" width=\"80%\"><tr class=\"trphpbg3\"><th colspan=\"11\">".$legende;
echo "</th></tr>\n<tr class=\"trphpbg3\">
<th align=\"center\">".CLMNT_POSITION."</th>
<th align=\"left\">".CLMNT_EQUIPE."</th>
<th align=\"left\">".CLMNT_POINTS."</th>\n";
...
echo "<table class=tablephpbg2 align=\"center\"  cellspacing=\"0\" width=\"95%\"><tr class=trphpbg3><th colspan=10>".$legendemini;
echo "<tr class=trphpbg3>
<th align=\"center\">".CLMNT_POSITION."
<th align=\"center\">".CLMNT_EQUIPE."
<th align=\"left\">".CLMNT_POINTS."
...
<form method="get" action=""><div align="center">
  ...
  <?php       
       $query = "SELECT phpbg_divisions.nom, phpbg_saisons.annee, phpbg_championnats.id
                 FROM phpbg_championnats, phpbg_divisions, phpbg_saisons
                 WHERE phpbg_championnats.id_division=phpbg_divisions.id
                 AND phpbg_championnats.id_saison=phpbg_saisons.id
                 ORDER BY phpbg_saisons.annee DESC, phpbg_championnats.id";
...
function demande_equipe($champ)
{
  ...
  $requete = "SELECT phpbg_clubs.nom, id_champ, phpbg_equipes.id
              FROM phpbg_clubs, phpbg_equipes
              WHERE phpbg_equipes.id_champ='$champ' and phpbg_clubs.id=phpbg_equipes.id_club
              ORDER BY nom";
$resultat = mysqli_query($requete) or die (mysqli_error());
