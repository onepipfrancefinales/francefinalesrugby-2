...
function nb_journees($id_champ, $idconnect)
         ...
         $reponse=$idconnect->query("SELECT id FROM phpbg_equipes WHERE id_champ='$id_champ'");
...
//suppression des liens sur les scores
  ...
  $requete="SELECT phpbg_clubs.nom, CLEXT.nom, phpbg_matchs.buts_dom, phpbg_matchs.buts_ext,
...
{
  $nb_requete=0;
  if($cache=="1"){ mysql_query("DELETE FROM phpbg_clmnt_cache WHERE ID_CHAMP='$champ'") or die (mysql_error());}
  else{ mysql_query("DELETE FROM phpbg_clmnt") or die (mysql_error());}
          $nb_requete++;
...
$query="SELECT pts_victoire, pts_nul, pts_defaite FROM phpbg_parametres WHERE id_champ='$champ'";
$nb_requete++;
...
// victoires domicile
$query="SELECT dom.id, count(dom.id), phpbg_clubs.nom, sum(buts_dom), sum(buts_ext) FROM phpbg_equipes as dom, phpbg_clubs, phpbg_matchs, phpbg_journees, phpbg_championnats
...
$query="SELECT phpbg_clubs.nom FROM phpbg_clubs, phpbg_equipes, phpbg_championnats
...
// victoires exterieur
$query="SELECT ext.id, count(ext.id), phpbg_clubs.nom, sum(buts_ext), sum(buts_dom) FROM phpbg_equipes as ext, phpbg_clubs, phpbg_matchs, phpbg_journees, phpbg_championnats
...
$query="SELECT phpbg_clubs.nom, phpbg_equipes.penalite, phpbg_equipes.id
...
//mysql_query("LOCK TABLE phpbg_clmnt WRITE, phpbg_equipes WRITE, phpbg_clubs WRITE") or die (mysql_error());
    ...
    {
    $query="SELECT phpbg_clubs.nom, phpbg_equipes.id 
            FROM phpbg_clubs, phpbg_equipes, phpbg_championnats
            WHERE phpbg_equipes.id_champ=phpbg_championnats.id
            AND phpbg_championnats.id='$champ'
            AND phpbg_equipes.id_club=phpbg_clubs.id";
...
{
echo "<table class=\"tablephpbg2\" align=\"center\" cellspacing=\"0\" width=\"95%\"><tr class=\"trphpbg3\"><th colspan=\"11\">".$legende."</th></tr>\n";
echo "<tr class=\"trphpbg3\">
