...
if ($action2=="1")
 ...
 // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $resultats=$idconnect->query("SELECT id_champ FROM phpbg_parametres WHERE id_champ='$champ'");
 ...
 $nb_resultats=mysqli_num_rows($resultats);
   ...
   if ($nb_resultats>0)
     {
       //Màj des paramètres
       mysqli_query ($idconnect,("UPDATE phpbg_parametres SET pts_victoire='$pts_victoire', pts_nul='$pts_nul', pts_defaite='$pts_defaite', accession='$accession', barrage='$barrage', relegation='$relegation', id_equipe_fetiche='$id_equipe_fetiche', fiches_clubs='$fiches_clubs', estimation='$estimation' WHERE id_champ='$champ'")) or die (mysqli_error($idconnect));
     }

   else
     {
       // Insertion des paramètres dans la bdd
       $requete="INSERT INTO phpbg_parametres (id_champ, pts_victoire, pts_nul, pts_defaite, accession, barrage, relegation,id_equipe_fetiche, fiches_clubs, estimation) VALUES ('$champ', '$pts_victoire', '$pts_nul', '$pts_defaite', '$accession', '$barrage', '$relegation', '$id_equipe_fetiche', '$fiches_clubs', '$estimation')";
...
if ($malus)
 {
  ...
  while($x<$y)
   {
	  //   $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');
    mysqli_query($idconnect, ("UPDATE phpbg_equipes SET penalite = '$malus[$x]'  WHERE id = '$id_equipe[$x]'")) or die(mysql_error($idconnect));
    $x++;
   }
 }
...
if ($pts_admin)
 {
  ...
  while($x<$y)
   {
    mysqli_query($idconnect, (" UPDATE phpbg_equipes 
...
<table class=phpbg width="80%">
            <tr>
              <td class=phpbg2 align="center" colspan="3"><?php echo ADMIN_PARAM_TITRE." "; affich_champ ($champ, $idconnect); ?></td>
            </tr>
            <tr>
              <td>
...
// $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $resultats=$idconnect->query("SELECT phpbg_clubs.nom, phpbg_equipes.id FROM phpbg_equipes, phpbg_clubs WHERE phpbg_equipes.id_champ='$champ' AND phpbg_clubs.id=phpbg_equipes.id_club ORDER by nom");
...
echo "</tr><tr>	<input type=\"hidden\" name=\"action2\" value=\"1\">
...
<!-- tableau points Bonus Malus-->

<table class=phpbg width="80%">
  <tr>
    <td class=phpbg2 align="center" colspan="3"><?php echo ADMIN_TAPVERT_TITRE." "; affich_champ ($champ, $idconnect); ?></td>
  </tr>
  <tr>
    <td><?php 
	...
	$result=$idconnect->query("SELECT DISTINCT phpbg_clubs.nom, phpbg_equipes.id, penalite 
...
<table class=phpbg width="80%">
  <tr>
    <td class=phpbg2 align="center" colspan="4"><?php echo "Points administratifs "; affich_champ ($champ, $idconnect); ?></td>
  </tr>
  <tr>
    <td><?php 
	...
	$result=$idconnect->query("	SELECT DISTINCT phpbg_clubs.nom, phpbg_equipes.id, phpbg_equipes.pts_admin, phpbg_equipes.jour_pere  
								FROM phpbg_clubs, phpbg_equipes 
								WHERE phpbg_equipes.id_champ='$champ' 
								AND phpbg_clubs.id=phpbg_equipes.id_club 
								ORDER BY phpbg_clubs.nom");
