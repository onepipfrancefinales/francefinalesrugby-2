<?php
//***********************************************************************/
// phpbgceeague : gestionnaire de championnat                              */
// ============================================                         */
//                                                                      */
// Version : 0.82                                                       */
// Copyright (c) 2004    Alexis MANGIN                                  */
// http://phpbgceeague.univert.org                                         */
//                                                                      */
...
//                                                                      */
//***********************************************************************/
// Support technique : http://phpbgceeague.univert.org/forum               */
//                                                                      */
//***********************************************************************/
...
{
  mysqli_query($idconnect, ("INSERT INTO phpbg_saisons (annee) values ('$saison')")) or die ("probleme " .mysqli_error());
}

if ($action2=="creer" and $division and $action=="creer")
{
  mysqli_query($idconnect, ("INSERT INTO phpbg_divisions (nom) values ('$division')")) or die ("probleme " .mysqli_error());
}

if ($action3=="creer" and $action=="creer")
{
  mysqli_query($idconnect, ("INSERT INTO phpbg_championnats (id_division, id_saison) values ('$division','$saison')")) or die ("probleme " .mysqli_error());
}


if ($action2=="supp" and $saison and $action=="creer")
{
  mysqli_query($idconnect, ("DELETE FROM phpbg_saisons WHERE id ='$saison'")) or die ("probleme " .mysqli_error());
}

if ($action2=="supp" and $division and $action=="creer")
{
  mysqli_query($idconnect, ("DELETE FROM phpbg_divisions WHERE id ='$division'")) or die ("probleme " .mysqli_error());
}

if ($confirm=="ok" and $champ and $action=="supp")
{
   $resultats=$idconnect->query("SELECT phpbg_equipes.id FROM phpbg_equipes, phpbg_championnats WHERE id_champ='$champ'");
 // $resultats=mysql_query($requete);
     while($row = mysqli_fetch_array($resultats))
      {
        mysqli_query($idconnect, ("DELETE FROM phpbg_matchs where id_equipe_dom='$row[0]' ")) or die ("probleme" .mysqli_error());
        mysqli_query($idconnect, ("DELETE FROM phpbg_matchs where id_equipe_ext='$row[0]' ")) or die ("probleme" .mysqli_error());
      }
    mysqli_query($idconnect, ("DELETE FROM phpbg_championnats where id='$champ' ")) or die ("probleme " .mysqli_error());
    mysqli_query($idconnect, ("DELETE FROM phpbg_journees WHERE  id_champ = '$champ'")) or die ("probleme " .mysqli_error());
    mysqli_query($idconnect, ("DELETE FROM phpbg_equipes WHERE id_champ ='$champ' ")) or die ("probleme " .mysqli_error());
    mysqli_query($idconnect, ("DELETE FROM phpbg_parametres WHERE id_champ ='$champ' ")) or die ("probleme " .mysqli_error());

}
?>

<br /><font class=phpbg><?php echo LEAGUE; ?></font><br /><br />



<table class=phpbg width="90%">
            <tr>
              
    <td class=phpbg2 width="6%"> 
      <?php //echo MENU_ID; ?>
    </td>
              
    <td class=phpbg2 width="36%"> 
      <?php //echo MENU_NOM; ?>
    </td>
              
    <td class=phpbg2 width="12%"> 
      <?php //echo ADMIN_JOURNEES_MSG3; ?>
    </td>
              
    <td class=phpbg2 width="46%"></td>
            </tr>
            <?php affich_championnatsMAJ ($champ, $action, $idconnect); ?>
            <tr>
              
    <td class=phpbg5 align="right" colspan="4"> 
      </td>
            </tr>
