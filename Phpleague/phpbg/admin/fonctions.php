...
{
  $resultats=$idconnect->query("SELECT phpbg_championnats.id, phpbg_divisions.nom, phpbg_saisons.annee 
    ...
    while ($row = mysqli_fetch_array($resultats))
      {
        ...
        if ($champ=="$row[0]") {$class="phpbg7";}
        elseif (($i%2)==0) {$class="phpbg3";}
        else {$class="phpbg4";}
    ...
    while ($row = mysqli_fetch_array($resultats))
      {
        ...
        if ($gr_champ=="$row[0]") {$class="phpbg7";}
        elseif (($i%2)==0) {$class="phpbg3";}
        else {$class="phpbg4";}
...
{
  $resultats=$idconnect->query("SELECT phpbg_divisions.nom, phpbg_saisons.annee 
              FROM phpbg_championnats, phpbg_divisions, phpbg_saisons
              WHERE phpbg_championnats.id_division=phpbg_divisions.id 
              AND phpbg_championnats.id_saison=phpbg_saisons.id 
              AND phpbg_championnats.id='$champ' ORDER by annee desc");
...
{
  $result=$idconnect->query(" SELECT phpbg_divisions.id, phpbg_divisions.nom 
							  FROM phpbg_divisions 
							  ORDER by nom");
...
{
  $result=$idconnect->query(" SELECT phpbg_saisons.id, phpbg_saisons.annee 
							  FROM phpbg_saisons 
							  ORDER by annee");
...
{    
  $result=$idconnect->query(" SELECT phpbg_clubs.id, phpbg_clubs.nom 
							  FROM phpbg_clubs 
							  ORDER by nom");
...
function champ_menu($idconnect)
{
 $result=$idconnect->query(" 
		 SELECT phpbg_championnats.id, phpbg_divisions.nom, phpbg_saisons.annee, (phpbg_saisons.annee)+1 
		 FROM phpbg_championnats, phpbg_divisions, phpbg_saisons 
		 WHERE phpbg_divisions.id=phpbg_championnats.id_division 
		 AND phpbg_saisons.id=phpbg_championnats.id_saison 
		 ORDER by annee desc, phpbg_divisions.nom");
...
{
  $result=$idconnect->query(" SELECT phpbg_equipes.id, phpbg_clubs.nom 
							  FROM phpbg_clubs, phpbg_equipes 
							  WHERE id_champ='$champ' 
							  AND phpbg_clubs.id=phpbg_equipes.id_club 
							  ORDER by nom");
    ...
    while ($row=mysqli_fetch_array($result))
    {
         $result2=$idconnect->query(" 
					SELECT phpbg_championnats.id, phpbg_divisions.nom, phpbg_saisons.annee, (phpbg_saisons.annee)+1 
					 ...
					 WHERE phpbg_divisions.id=phpbg_championnats.id_division 
					 AND phpbg_saisons.id=phpbg_championnats.id_saison 
					 AND phpbg_championnats.id='$row[0]'  
					 ORDER by annee desc, phpbg_divisions.nom");
...
{ 
 $resultats= $idconnect->query("SELECT phpbg_clubs.nom, CLEXT.nom, phpbg_matchs.buts_dom, phpbg_matchs.buts_ext, phpbg_matchs.id, phpbg_matchs.date_reelle
  ...
  $resultats3=$idconnect->query(" SELECT phpbg_clubs.nom, CLEXT.nom, phpbg_matchs.buts_dom, phpbg_matchs.buts_ext, phpbg_matchs.id, phpbg_matchs.date_reelle 
								  FROM phpbg_clubs, phpbg_clubs as CLEXT, phpbg_matchs, phpbg_journees, phpbg_equipes, phpbg_equipes as EXT 
								  WHERE phpbg_clubs.id=phpbg_equipes.id_club 
								  AND CLEXT.id=EXT.id_club 
								  AND phpbg_equipes.id=phpbg_matchs.id_equipe_dom 
...
function nom_club($id_equipe, $idconnect)
{
  $result=$idconnect->query(" SELECT nom 
							  FROM phpbg_clubs, phpbg_equipes 
							  WHERE phpbg_clubs.id=phpbg_equipes.id_club 
							  and phpbg_equipes.id='$id_equipe'");
...
function nb_matchs ($numero, $champ, $idconnect)
{
  $result=$idconnect->query("select * 
							 from phpbg_matchs, phpbg_journees 
							 where phpbg_matchs.id_journee=phpbg_journees.id 
							 and phpbg_journees.numero=$numero 
							 and phpbg_journees.id_champ=$champ");
...
function db_clmnt($champ, $debut, $fin, $cache, $idconnect)
{
  ...
  if($cache=="1")
	{ 
	 mysqli_query($idconnect,("DELETE FROM phpbg_clmnt_cache WHERE ID_CHAMP='$champ'")) or die (mysqli_error($idconnect));
	}
	else
	{ 
	 mysqli_query($idconnect,("DELETE FROM phpbg_clmnt")) or die (mysqli_error($idconnect));
	}
     $nb_requete++;
	...
	$dom=$idconnect->query("SELECT dom.id, count(dom.id), phpbg_clubs.nom, sum(buts_dom), sum(buts_ext) 
						  FROM phpbg_equipes as dom, phpbg_clubs, phpbg_matchs, phpbg_journees, phpbg_championnats
	...
	$dom=$idconnect->query("
		 SELECT dom.id, count(dom.id), phpbg_clubs.nom, sum(buts_dom), sum(buts_ext) 
	     FROM phpbg_equipes as dom, phpbg_clubs, phpbg_matchs, phpbg_journees, phpbg_championnats
		 WHERE dom.id_champ='$champ'
         AND dom.id_club=phpbg_clubs.id
         AND dom.id=phpbg_matchs.id_equipe_dom
   ...
   $result=$idconnect->query("SELECT phpbg_clubs.nom 
...
// TABLEAU DE CLASSEMENT
  ...
  $result=$idconnect->query("SELECT phpbg_clubs.nom, phpbg_equipes.penalite, phpbg_equipes.id,phpbg_equipes.pts_admin, phpbg_equipes.jour_pere
							 FROM phpbg_clubs, phpbg_equipes, phpbg_championnats
							 WHERE phpbg_equipes.id_champ=phpbg_championnats.id
							 AND phpbg_championnats.id='$champ'
							 AND phpbg_equipes.id_club=phpbg_clubs.id");
	...
	*/
	//mysqlY_query("LOCK TABLE phpbg_clmnt WRITE, phpbg_equipes WRITE, phpbg_clubs WRITE") or die (mysql_error());
	/*
	...
	*/
    while($row = mysqli_fetch_array($result))
    {
		...
		$POINTS= $DOMPOINTS+ $EXTPOINTS + $row['penalite'] + $row['pts_admin'];
		...
		$PEN = $row['penalite'];
		$PTSADMIN = $row['pts_admin'];
		$JOURPERE = $row['jour_pere'];
		$NOM=addslashes($row['nom']);
//echo $NOM.' - '.$POINTS;echo "<br />";
		if($cache=="1")
		{
		 mysqli_query($idconnect, ("INSERT INTO phpbg_clmnt_cache(NOM, POINTS, JOUES,  G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, PTS_ADMIN, DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
									  VALUES('$NOM','$POINTS','$JOUES', '$G','$N','$P','$BUTSPOUR','$BUTSCONTRE','$DIFF','$PEN', '$PTSADMIN', '$DOMPOINTS','$DOMJOUES','$DOMG','$DOMN','$DOMP','$DOMBUTSPOUR','$DOMBUTSCONTRE','$DOMDIFF','$EXTPOINTS','$EXTJOUES','$EXTG','$EXTN','$EXTP','$EXTBUTSPOUR','$EXTBUTSCONTRE','$EXTDIFF','$ID_EQUIPE','$ID_CHAMP')"));

		}
		else
		{
		 mysqli_query($idconnect, ("INSERT INTO phpbg_clmnt(NOM, POINTS, JOUES, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, PTS_ADMIN, DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
									  VALUES('$NOM','$POINTS','$JOUES', '$G','$N','$P','$BUTSPOUR','$BUTSCONTRE','$DIFF','$PEN', '$PTSADMIN' ,'$DOMPOINTS','$DOMJOUES','$DOMG','$DOMN','$DOMP','$DOMBUTSPOUR','$DOMBUTSCONTRE','$DOMDIFF','$EXTPOINTS','$EXTJOUES','$EXTG','$EXTN','$EXTP','$EXTBUTSPOUR','$EXTBUTSCONTRE','$EXTDIFF','$ID_EQUIPE','$ID_CHAMP')"));
	...
	if($cache=="1")
	{$requete="DELETE FROM phpbg_clmnt_cache WHERE nom='exempte'" or die (mysql_error($idconnect));}
	else
	{$requete="DELETE FROM phpbg_clmnt WHERE nom='exempte'" or die (mysql_error($idconnect));}
...
// Nombres de renseignement dans cette classe � partir de l'id_classe  (utilis� dans phpbg_classe.php)
function nb_classe($data, $idconnect)
{
  $result=$idconnect->query("SELECT id FROM phpbg_rens WHERE id_classe='$data'");
//if (!$result) die mysql_error();
  $nb_classe=mysqli_num_rows( $result );
  return("$nb_classe");
}

// Nombres de classes enregistr�es (ulilis� dans phpbg_classe.php)
function nb_classe2($idconnect)
{	
  $result=$idconnect->query("SELECT * FROM phpbg_classe");
//if (!$result) die mysql_error();
...
{
  $result=$idconnect->query("SELECT id FROM phpbg_rens where id_classe>'0'");
...
{	
  $result=$idconnect->query("SELECT * FROM phpbg_rens");
...
{
  $result=$idconnect->query("select id, nom from phpbg_rens where nom='$rens'");
//if (!$result) die mysql_error();
  $row=mysqli_fetch_array($result);
  $rens2=$row[0];
  return("$rens2");
...
function aff_rens ($id_classe, $id_clubs, $idconnect)
  {
  $result=$idconnect->query("SELECT phpbg_donnee.id, phpbg_donnee.nom, id_rens, id_clubs, phpbg_rens.id, phpbg_rens.nom, phpbg_rens.id_classe, phpbg_clubs.id, etat, phpbg_donnee.url, phpbg_rens.url
