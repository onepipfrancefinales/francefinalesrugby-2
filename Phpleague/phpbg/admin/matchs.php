...
if (empty ($numero))
  {
    $result=$idconnect->query("
			SELECT max(phpbg_journees.numero) 
...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journée
        ...
        $query = "SELECT DISTINCT phpbg_clubs.nom, phpbg_equipes.id FROM phpbg_clubs, phpbg_equipes
	   ...
	   $result= $idconnect->query("
				SELECT DISTINCT phpbg_clubs.nom, phpbg_equipes.id 
				FROM phpbg_clubs, phpbg_equipes 
				WHERE phpbg_equipes.id_champ='$champ' 
				AND phpbg_clubs.id=phpbg_equipes.id_club 
				ORDER BY phpbg_clubs.nom");
...
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $result=$idconnect->query("
			SELECT count(DISTINCT phpbg_matchs.id_equipe_dom), 
			count(DISTINCT phpbg_matchs.id_equipe_ext) 
			FROM phpbg_matchs, phpbg_journees   
			WHERE phpbg_matchs.id_journee=phpbg_journees.id    
			AND phpbg_journees.id_champ='$champ'    
			AND phpbg_journees.numero='$x' ");
			//$result=mysql_query($query);
...
$result=$idconnect->query(" 
		SELECT sum(phpbg_matchs.id_equipe_dom), 
			   sum(phpbg_matchs.id_equipe_ext) 
		FROM phpbg_matchs, phpbg_journees 
		WHERE phpbg_matchs.id_journee=phpbg_journees.id 
		AND phpbg_journees.id_champ='$champ'  ");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT   phpbg_matchs.id_equipe_dom as DOM,  count(*) as ct 
		 FROM phpbg_matchs, phpbg_journees 
		 WHERE  phpbg_journees.id_champ='$champ' 
		 AND phpbg_matchs.id_journee=phpbg_journees.id 
		 GROUP BY DOM");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT phpbg_matchs.id_equipe_ext as DOM,  
		 count(*) as ct 
		 FROM phpbg_matchs, phpbg_journees 
		 WHERE  phpbg_journees.id_champ='$champ' 
		 AND phpbg_matchs.id_journee=phpbg_journees.id 
