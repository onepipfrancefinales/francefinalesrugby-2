...
//								 FROM phpfed3NE_pronostics WHERE id_match='$val_matchs_id'");
          
 //          while ($row2=mysqli_fetch_array($resultats2))
 //          {
            //  if ($buts_dom >$buts_ext and $row2[0]=="1"){$query3="UPDATE phpfed3NE_pronostics SET points='$points_prono_exact', participation='1' WHERE id_membre='$row2[1]' AND id_match='$val_matchs_id'";}
             // elseif ($buts_dom ==$buts_ext and $row2[0]=="N"){$query3="UPDATE phpfed3NE_pronostics SET points='$points_prono_exact', participation='1' WHERE id_membre='$row2[1]' AND id_match='$val_matchs_id'";}
            //  elseif ($buts_dom <$buts_ext and $row2[0]=="2"){$query3="UPDATE phpfed3NE_pronostics SET points='$points_prono_exact', participation='1' WHERE id_membre='$row2[1]' AND id_match='$val_matchs_id'";}
             // else 
			 // {
    ...
    if ( !(($val_butd=='') or ($val_butv=='')))
       {
        ...
        echo "Etape 1 : affichages des scores"."<br />";
        //Insertion des données dans la table matchs
		mysqli_query ($idconnect, ("UPDATE phpfed3NE_matchs  SET phpfed3NE_matchs.buts_dom='$val_butd', phpfed3NE_matchs.buts_ext='$val_butv', phpfed3NE_matchs.date_reelle='$date_us'
                     WHERE phpfed3NE_matchs.id='$val_matchs_id'"));
					 
		/*	
		//Insertion des données dans la table pronostics	
		$requetePronos="UPDATE phpfed3NE_pronostics SET buts_dom='$val_butd', buts_ext='$val_butv'
		...
		$resultats2=$idconnect->query("SELECT pronostic, id_membre 
           ...
           while ($row2=mysqli_fetch_array($resultats2))
           {
              if ($val_butd>$val_butv and $row2[0]=="1"){$query3="UPDATE phpfed3NE_pronostics SET points='$points_prono_exact', participation='1' WHERE id_membre='$row2[1]' AND id_match='$val_matchs_id'";}
              elseif ($val_butd==$val_butv and $row2[0]=="N"){$query3="UPDATE phpfed3NE_pronostics SET points='$points_prono_exact', participation='1' WHERE id_membre='$row2[1]' AND id_match='$val_matchs_id'";}
              elseif ($val_butd<$val_butv and $row2[0]=="2"){$query3="UPDATE phpfed3NE_pronostics SET points='$points_prono_exact', participation='1' WHERE id_membre='$row2[1]' AND id_match='$val_matchs_id'";}
              else 
			  {
			 mysqli_query ($idconnect, ("UPDATE phpfed3NE_pronostics 
						SET points='$pts_prono_participation', participation='1' 
       ...
       */   	 
       ...
       {
        echo "Etape 2 : absences des scores"."<br />";  

 mysqli_query ($idconnect, ("UPDATE phpfed3NE_matchs 
					SET phpfed3NE_matchs.buts_dom = NULL, phpfed3NE_matchs.buts_ext = NULL, phpfed3NE_matchs.date_reelle='$date_us'
                    WHERE phpfed3NE_matchs.id ='$val_matchs_id' ")) ;	
  /*
         $resultats2=$idconnect->query("SELECT pronostic, id_membre 
										FROM phpfed3NE_pronostics 
