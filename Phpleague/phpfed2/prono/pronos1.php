...
$resultat=$idconnect->query("SELECT phpfed2_matchs.id
  ...
  //$resultat=mysql_query($requete) or die ("probleme " .mysql_error());
      while ($row=mysqli_fetch_array($resultat))
      {
        $resultat1=$idconnect->query("SELECT phpfed2_matchs.date_reelle FROM phpfed2_matchs WHERE phpfed2_matchs.id='$row[0]'");
       // $resultat1=mysql_query($requete1);
        ...
        {
           mysqli_query($idconnect, ("UPDATE  phpfed2_pronostics SET pronostic='0' WHERE id_match='$row[0]' AND id_membre='$id'"));}
           mysqli_query($idconnect, ("DELETE FROM phpfed2_pronostics WHERE pronostic='0'"));
        }
      }
  ...
  if ($action == "valid_pronos")
  {
    ...
    {   
     ...
     $resultat=$idconnect->query("SELECT phpfed2_matchs.date_reelle FROM phpfed2_matchs WHERE phpfed2_matchs.id='$id_match[$i]'");
    // $resultat=mysql_query($requete);
     ...
     $resultat=$idconnect->query("SELECT tps_avant_prono FROM phpfed2_gr_championnats WHERE id='$gr_champ'");
     //$resultat=mysql_query($requete);
     ...
     {
       mysqli_query($idconnect, ("DELETE FROM phpfed2_pronostics WHERE pronostic=' '"));
       $resultat =$idconnect->query( "SELECT * FROM phpfed2_matchs, phpfed2_pronostics, phpfed2_membres WHERE phpfed2_membres.id_prono='$user_id'
                   AND phpfed2_membres.id=phpfed2_pronostics.id_membre
                   AND phpfed2_pronostics.id_match=phpfed2_matchs.id
                   AND phpfed2_pronostics.id_match='$id_match[$i]'");
      // $resultat=mysql_query($requete);
       $nb_prono=mysqli_num_rows($resultat);

       $resultat = $idconnect->query("SELECT id FROM phpfed2_membres WHERE id_prono='$user_id'");
       //$resultat = mysql_query($requete);
       ...
       if ($nb_prono == "1")
       {
         if ($date_actuelle<($date_match_timestamp+$temps_avant_prono*60))
         {
           mysqli_query($idconnect, ("UPDATE phpfed2_pronostics SET pronostic='$f_prono[$i]'
                        WHERE phpfed2_pronostics.id_membre='$id'
                        AND phpfed2_pronostics.id_match='$id_match[$i]'"));
         }
       }
       if ($nb_prono == "0")
       {
         if ($date_actuelle<($date_match_timestamp+$temps_avant_prono*60))
         {
           mysqli_query($idconnect, ("INSERT INTO phpfed2_pronostics (id_membre, pronostic, id_match, id_champ) VALUES ('$id','$f_prono[$i]','$id_match[$i]', '$gr_champ')"));
         }
       }
  ...
  {    if ($debut=="0") 
    ...
    $resultat=$idconnect->query("
			 SELECT phpfed2_clubs.nom, CLEXT.nom, phpfed2_matchs.id, phpfed2_matchs.date_reelle, phpfed2_journees.numero
			 FROM phpfed2_clubs, phpfed2_clubs as CLEXT, phpfed2_matchs, phpfed2_journees, phpfed2_equipes, phpfed2_equipes as EXT, phpfed2_gr_championnats
			 WHERE phpfed2_clubs.id=phpfed2_equipes.id_club
			 AND CLEXT.id=EXT.id_club
			 AND phpfed2_equipes.id=phpfed2_matchs.id_equipe_dom
    ...
    if (mysqli_num_rows($resultat)=="0") 
      ...
      while ($row=mysqli_fetch_array($resultat) and $i<$nb_matchs)
      {
       ...
       $resultat2=$idconnect->query( " SELECT pronostic 
									   FROM phpfed2_pronostics, phpfed2_membres 
									   WHERE phpfed2_pronostics.id_match='$row[2]' 
									   AND phpfed2_membres.id=phpfed2_pronostics.id_membre 
									   AND phpfed2_membres.id_prono='$user_id'");
       ...
       $resultat2=$idconnect->query("SELECT tps_avant_prono FROM phpfed2_gr_championnats WHERE id='$gr_champ'");
      //$resultat2=mysql_query($requete2) or die ("probleme " .mysql_error());
