...
function affiche_points ($user_id, $gr_champ, $idconnect)
{
 // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  ...
  $result=$idconnect->query("SELECT points 
						    FROM phpfed2_clmnt_pronos, phpfed2_membres 
						    WHERE phpfed2_membres.id=phpfed2_clmnt_pronos.id_membre 
						    and phpfed2_membres.id_prono='$user_id' and type='general' 
...
function affiche_clmnt_general ($user_id, $gr_champ, $idconnect)
{
  ...
  $result=$idconnect->query(" SELECT id_prono 
							  ...
							  AND type='general' 
							  AND id_champ='$gr_champ' 
							  ORDER by points desc, participation desc, phpfed2_membres.pseudo");
  //$result=mysql_query($query) or die (mysql_error());
  $i = "1";
  while ($row=mysqli_fetch_array($result))
  {
...
$result=$idconnect->query("SELECT phpfed2_clubs.nom, CLEXT.nom, phpfed2_matchs.buts_dom, phpfed2_matchs.buts_ext, phpfed2_matchs.id, phpfed2_matchs.date_reelle, phpfed2_journees.numero
FROM phpfed2_clubs, phpfed2_clubs as CLEXT, phpfed2_matchs, phpfed2_journees, phpfed2_equipes, phpfed2_equipes as EXT, phpfed2_gr_championnats
...
AND phpfed2_journees.id_champ=phpfed2_gr_championnats.id_champ
AND phpfed2_gr_championnats.id='$gr_champ'
AND phpfed2_matchs.buts_dom is null
AND phpfed2_matchs.buts_ext is null
AND phpfed2_clubs.nom!='exempte'
AND CLEXT.nom!='exempte'
ORDER by phpfed2_matchs.date_reelle, phpfed2_clubs.nom
LIMIT 0, 10");
...
$result2=$idconnect->query( "SELECT pronostic 
							FROM phpfed2_pronostics, phpfed2_gr_championnats 
							WHERE phpfed2_pronostics.id_match='$row[4]' 
							AND phpfed2_gr_championnats.id='$gr_champ' 
...
if ($nb_pronos == "0") {$prono="0";}
{
  ...
  $resultats2=$idconnect->query("SELECT tps_avant_prono FROM phpfed2_gr_championnats WHERE id='$gr_champ'");
...
{
   mysqli_query($idconnect, ("DELETE FROM phpfed2_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_en_cours'"));

   $result=$idconnect->query("SELECT id_membre, pseudo, sum(points) as total, sum(participation) as participations
   FROM phpfed2_membres, phpfed2_pronostics, phpfed2_matchs, phpfed2_gr_championnats
   WHERE phpfed2_pronostics.id_champ=phpfed2_gr_championnats.id
   AND phpfed2_gr_championnats.id='$gr_champ'
   AND id_membre=phpfed2_membres.id
 ...
 //  $result=mysql_query ($query) or die ("probleme " .mysql_error());
   while ($row=mysqli_fetch_array($result))
         ...
         mysqli_query($idconnect, ("INSERT INTO phpfed2_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_en_cours')"));
         }
}          

if ($type=="mensuel_30_jours")
{
   mysqli_query($idconnect, ("DELETE FROM phpfed2_clmnt_pronos WHERE id_champ='$gr_champ' AND type='mensuel_30_jours'"));
   ...
   //$result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       mysqli_query($idconnect, ("INSERT INTO phpfed2_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) values ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'mensuel_30_jours')"));
       }
}

if ($type=="hebdo")
{
   mysqli_query($idconnect, ("DELETE FROM phpfed2_clmnt_pronos WHERE id_champ='$gr_champ' AND type='hebdo'"));
  ...
  // $result=mysql_query ($query) or die ("probleme " .mysql_error());
       ...
       {
       $row[1]=addslashes($row[1]);
       mysqli_query($idconnect, ("INSERT INTO phpfed2_clmnt_pronos (id_champ, id_membre, pseudo, points, participation, type) 
								  VALUES ('$gr_champ', '$row[0]', '$row[1]', '$row[2]', '$row[3]', 'hebdo')"));
       }
}
if (isset($_REQUEST['complet'])) {$complet=$_REQUEST['complet'];} else {$complet='';}

$result=$idconnect->query("SELECT pseudo, points, participation FROM phpfed2_clmnt_pronos
...
function pseudo_admin ($gr_champ, $idconnect)
{
 $resultat=$idconnect->query("SELECT pseudo 
							  FROM phpfed2_membres, phpfed2_gr_championnats 
							  WHERE phpfed2_gr_championnats.id_master=phpfed2_membres.id 
							  AND phpfed2_gr_championnats.id='$gr_champ'");
...
function champ_prono ($gr_champ, $idconnect)
{
 ...
 $resultat=$idconnect->query("SELECT DISTINCT id, nom FROM phpfed2_gr_championnats WHERE  phpfed2_gr_championnats.activ_prono='1' ORDER by id");
