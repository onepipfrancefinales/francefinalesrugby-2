...
{
  $resultats=$idconnect->query("SELECT phpfed2_championnats.id, phpfed2_divisions.nom, phpfed2_saisons.annee 
    ...
    while ($row = mysqli_fetch_array($resultats))
      {
        ...
        if ($champ=="$row[0]") {$class="phpfed27";}
        elseif (($i%2)==0) {$class="phpfed23";}
        else {$class="phpfed24";}
    ...
    while ($row = mysqli_fetch_array($resultats))
      {
        ...
        if ($gr_champ=="$row[0]") {$class="phpfed27";}
        elseif (($i%2)==0) {$class="phpfed23";}
        else {$class="phpfed24";}
...
{
  $resultats=$idconnect->query("SELECT phpfed2_divisions.nom, phpfed2_saisons.annee 
              FROM phpfed2_championnats, phpfed2_divisions, phpfed2_saisons
              WHERE phpfed2_championnats.id_division=phpfed2_divisions.id 
              AND phpfed2_championnats.id_saison=phpfed2_saisons.id 
              AND phpfed2_championnats.id='$champ' ORDER by annee desc");
...
{
  $result=$idconnect->query(" SELECT phpfed2_divisions.id, phpfed2_divisions.nom 
							  FROM phpfed2_divisions 
							  ORDER by nom");
...
{
  $result=$idconnect->query(" SELECT phpfed2_saisons.id, phpfed2_saisons.annee 
...
{    
  $result=$idconnect->query(" SELECT phpfed2_clubs.id, phpfed2_clubs.nom 
							  FROM phpfed2_clubs 
							  ORDER by nom");
...
function champ_menu($idconnect)
{
 $result=$idconnect->query(" 
		 SELECT phpfed2_championnats.id, phpfed2_divisions.nom, phpfed2_saisons.annee, (phpfed2_saisons.annee)+1 
		 FROM phpfed2_championnats, phpfed2_divisions, phpfed2_saisons 
		 WHERE phpfed2_divisions.id=phpfed2_championnats.id_division 
		 AND phpfed2_saisons.id=phpfed2_championnats.id_saison 
		 ORDER by annee desc, phpfed2_divisions.nom");
...
{
  $result=$idconnect->query(" SELECT phpfed2_equipes.id, phpfed2_clubs.nom 
							  FROM phpfed2_clubs, phpfed2_equipes 
							  WHERE id_champ='$champ' 
							  AND phpfed2_clubs.id=phpfed2_equipes.id_club 
							  ORDER by nom");
    ...
    while ($row=mysqli_fetch_array($result))
    {
         $result2=$idconnect->query(" 
					SELECT phpfed2_championnats.id, phpfed2_divisions.nom, phpfed2_saisons.annee, (phpfed2_saisons.annee)+1 
					 ...
					 WHERE phpfed2_divisions.id=phpfed2_championnats.id_division 
					 AND phpfed2_saisons.id=phpfed2_championnats.id_saison 
					 AND phpfed2_championnats.id='$row[0]'  
					 ORDER by annee desc, phpfed2_divisions.nom");
...
{ 
 $resultats= $idconnect->query("SELECT phpfed2_clubs.nom, CLEXT.nom, phpfed2_matchs.buts_dom, phpfed2_matchs.buts_ext, phpfed2_matchs.id, phpfed2_matchs.date_reelle
  ...
  $resultats3=$idconnect->query(" SELECT phpfed2_clubs.nom, CLEXT.nom, phpfed2_matchs.buts_dom, phpfed2_matchs.buts_ext, phpfed2_matchs.id, phpfed2_matchs.date_reelle 
								  FROM phpfed2_clubs, phpfed2_clubs as CLEXT, phpfed2_matchs, phpfed2_journees, phpfed2_equipes, phpfed2_equipes as EXT 
								  WHERE phpfed2_clubs.id=phpfed2_equipes.id_club 
								  AND CLEXT.id=EXT.id_club 
								  AND phpfed2_equipes.id=phpfed2_matchs.id_equipe_dom 
...
function nom_club($id_equipe, $idconnect)
{
  $result=$idconnect->query(" SELECT nom 
							  FROM phpfed2_clubs, phpfed2_equipes 
							  WHERE phpfed2_clubs.id=phpfed2_equipes.id_club 
							  and phpfed2_equipes.id='$id_equipe'");
...
function nb_matchs ($numero, $champ, $idconnect)
{
  $result=$idconnect->query("select * 
							 from phpfed2_matchs, phpfed2_journees 
							 where phpfed2_matchs.id_journee=phpfed2_journees.id 
							 and phpfed2_journees.numero=$numero 
							 and phpfed2_journees.id_champ=$champ");
...
function db_clmnt($champ, $debut, $fin, $cache, $idconnect)
{
  ...
  if($cache=="1")
	{ 
	 mysqli_query($idconnect,("DELETE FROM phpfed2_clmnt_cache WHERE ID_CHAMP='$champ'")) or die (mysqli_error($idconnect));
	}
	else
	{ 
	 mysqli_query($idconnect,("DELETE FROM phpfed2_clmnt")) or die (mysqli_error($idconnect));
	}
     $nb_requete++;
	...
	$dom=$idconnect->query("SELECT dom.id, count(dom.id), phpfed2_clubs.nom, sum(buts_dom), sum(buts_ext) 
						  FROM phpfed2_equipes as dom, phpfed2_clubs, phpfed2_matchs, phpfed2_journees, phpfed2_championnats
	...
	$dom=$idconnect->query("
		 SELECT dom.id, count(dom.id), phpfed2_clubs.nom, sum(buts_dom), sum(buts_ext) 
	     FROM phpfed2_equipes as dom, phpfed2_clubs, phpfed2_matchs, phpfed2_journees, phpfed2_championnats
		 WHERE dom.id_champ='$champ'
         AND dom.id_club=phpfed2_clubs.id
         AND dom.id=phpfed2_matchs.id_equipe_dom
   ...
   $result=$idconnect->query("SELECT phpfed2_clubs.nom 
...
// TABLEAU DE CLASSEMENT
  ...
  $result=$idconnect->query("SELECT phpfed2_clubs.nom, phpfed2_equipes.penalite, phpfed2_equipes.id
	...
	*/
	//mysqlY_query("LOCK TABLE phpfed2_clmnt WRITE, phpfed2_equipes WRITE, phpfed2_clubs WRITE") or die (mysql_error());
	/*
    if (mysqli_num_rows($result)==0)
    {
	   // $idconnect=@mysqli_connect('127.0.0.1','root','','onepip-france-db3');	
  
		$result=$idconnect->query("SELECT phpfed2_clubs.nom, phpfed2_equipes.id 
            FROM phpfed2_clubs, phpfed2_equipes, phpfed2_championnats
            WHERE phpfed2_equipes.id_champ=phpfed2_championnats.id
            AND phpfed2_championnats.id='$champ'
            AND phpfed2_equipes.id_club=phpfed2_clubs.id");
...
echo $cache;
		if($cache=="1")
		{
		 mysqli_query($idconnect, ("INSERT INTO phpfed2_clmnt_cache(NOM, POINTS, JOUES, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
		...
		{
		 mysqli_query($idconnect, ("INSERT INTO phpfed2_clmnt(NOM, POINTS, JOUES, G, N, P, BUTSPOUR, BUTSCONTRE, DIFF, PEN, DOMPOINTS, DOMJOUES, DOMG, DOMN, DOMP, DOMBUTSPOUR, DOMBUTSCONTRE, DOMDIFF, EXTPOINTS, EXTJOUES, EXTG, EXTN, EXTP, EXTBUTSPOUR, EXTBUTSCONTRE, EXTDIFF, ID_EQUIPE, ID_CHAMP) 
									  VALUES('$NOM','$POINTS','$JOUES','$G','$N','$P','$BUTSPOUR','$BUTSCONTRE','$DIFF','$PEN','$DOMPOINTS','$DOMJOUES','$DOMG','$DOMN','$DOMP','$DOMBUTSPOUR','$DOMBUTSCONTRE','$DOMDIFF','$EXTPOINTS','$EXTJOUES','$EXTG','$EXTN','$EXTP','$EXTBUTSPOUR','$EXTBUTSCONTRE','$EXTDIFF','$ID_EQUIPE','$ID_CHAMP')"));
	...
	if($cache=="1")
	{$requete="DELETE FROM phpfed2_clmnt_cache WHERE nom='exempte'" or die (mysql_error($idconnect));}
	else
	{$requete="DELETE FROM phpfed2_clmnt WHERE nom='exempte'" or die (mysql_error($idconnect));}
...
// Nombres de renseignement dans cette classe à partir de l'id_classe  (utilisé dans phpfed2_classe.php)
function nb_classe($data, $idconnect)
{
  $result=$idconnect->query("SELECT id FROM phpfed2_rens WHERE id_classe='$data'");
//if (!$result) die mysql_error();
  $nb_classe=mysqli_num_rows( $result );
  return("$nb_classe");
}

// Nombres de classes enregistrées (ulilisé dans phpfed2_classe.php)
function nb_classe2($idconnect)
{	
  $result=$idconnect->query("SELECT * FROM phpfed2_classe");
//if (!$result) die mysql_error();
...
{
  $result=$idconnect->query("SELECT id FROM phpfed2_rens where id_classe>'0'");
...
{	
  $result=$idconnect->query("SELECT * FROM phpfed2_rens");
...
{
  $result=$idconnect->query("select id, nom from phpfed2_rens where nom='$rens'");
//if (!$result) die mysql_error();
  $row=mysqli_fetch_array($result);
  $rens2=$row[0];
  return("$rens2");
...
function aff_rens ($id_classe, $id_clubs, $idconnect)
  {
  $result=$idconnect->query("SELECT phpfed2_donnee.id, phpfed2_donnee.nom, id_rens, id_clubs, phpfed2_rens.id, phpfed2_rens.nom, phpfed2_rens.id_classe, phpfed2_clubs.id, etat, phpfed2_donnee.url, phpfed2_rens.url
