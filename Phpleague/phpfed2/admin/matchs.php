...
if (empty ($numero))
  {
    $result=$idconnect->query("
			SELECT max(phpfed2_journees.numero) 
...
for ($counter=$nb_rencontres; $counter>0 ; $counter=$counter-1 ) // Nb de rencontres dans la journée
        ...
        $query = "SELECT DISTINCT phpfed2_clubs.nom, phpfed2_equipes.id FROM phpfed2_clubs, phpfed2_equipes
	   ...
	   $result= $idconnect->query("
				SELECT DISTINCT phpfed2_clubs.nom, phpfed2_equipes.id 
				FROM phpfed2_clubs, phpfed2_equipes 
				WHERE phpfed2_equipes.id_champ='$champ' 
				AND phpfed2_clubs.id=phpfed2_equipes.id_club 
				ORDER BY phpfed2_clubs.nom");
...
echo "<small>MATCH_SUM = ".$sum_day."  </small>";
    $result=$idconnect->query("
			SELECT count(DISTINCT phpfed2_matchs.id_equipe_dom), 
			count(DISTINCT phpfed2_matchs.id_equipe_ext) 
			FROM phpfed2_matchs, phpfed2_journees   
			WHERE phpfed2_matchs.id_journee=phpfed2_journees.id    
			AND phpfed2_journees.id_champ='$champ'    
			AND phpfed2_journees.numero='$x' ");
			//$result=mysql_query($query);
...
$result=$idconnect->query(" 
		SELECT sum(phpfed2_matchs.id_equipe_dom), 
			   sum(phpfed2_matchs.id_equipe_ext) 
		FROM phpfed2_matchs, phpfed2_journees 
		WHERE phpfed2_matchs.id_journee=phpfed2_journees.id 
		AND phpfed2_journees.id_champ='$champ'  ");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT   phpfed2_matchs.id_equipe_dom as DOM,  count(*) as ct 
		 FROM phpfed2_matchs, phpfed2_journees 
		 WHERE  phpfed2_journees.id_champ='$champ' 
		 AND phpfed2_matchs.id_journee=phpfed2_journees.id 
		 GROUP BY DOM");
...
while($row=mysqli_fetch_array($result))
 ...
 $result=$idconnect->query("
		 SELECT phpfed2_matchs.id_equipe_ext as DOM,  
		 count(*) as ct 
		 FROM phpfed2_matchs, phpfed2_journees 
		 WHERE  phpfed2_journees.id_champ='$champ' 
		 AND phpfed2_matchs.id_journee=phpfed2_journees.id 
