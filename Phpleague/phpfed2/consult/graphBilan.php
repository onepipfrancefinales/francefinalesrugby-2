...
$result=$idconnect->query("SELECT phpfed2_clubs.nom, phpfed2_equipes.id_club, phpfed2_equipes.id_champ FROM phpfed2_equipes, phpfed2_clubs where phpfed2_clubs.id=phpfed2_equipes.id_club AND phpfed2_equipes.id='$equipe'");
...
$result=$idconnect->query("SELECT phpfed2_equipes.id FROM phpfed2_equipes, phpfed2_clubs WHERE id_champ='$champ' AND phpfed2_equipes.id_club=phpfed2_clubs.id AND phpfed2_clubs.nom!='exempte'");
//$result=mysql_query($query);
$nb_equipes=mysqli_num_rows($result)+1;

// on détermine la dernière journee jouée
$result=$idconnect->query("SELECT max(phpfed2_journees.numero) FROM phpfed2_journees, phpfed2_matchs where phpfed2_journees.id=phpfed2_matchs.id_journee and buts_dom is not NULL AND phpfed2_journees.id_champ='$champ'");
//$result=mysql_query($query);
while ($row=mysqli_fetch_array($result)){$fin=$row[0];}

// On détermine le nombre de journée total
$result=$idconnect->query("SELECT id FROM phpfed2_journees WHERE id_champ='$champ'");
//$result=mysql_query($query);
$nb_journees=mysqli_num_rows($result);

// On déterrmine l'année et le nom de la division
$result=$idconnect->query("SELECT phpfed2_saisons.annee, phpfed2_divisions.nom FROM phpfed2_saisons, phpfed2_divisions, phpfed2_championnats where phpfed2_championnats.id=$champ AND phpfed2_divisions.id=phpfed2_championnats.id_division and phpfed2_championnats.id_saison=phpfed2_saisons.id");
//$result=mysql_query($query);
...
$requete_sql = "SELECT classement FROM phpfed2_clmnt_graph WHERE id_equipe='$equipe' ORDER BY fin";
