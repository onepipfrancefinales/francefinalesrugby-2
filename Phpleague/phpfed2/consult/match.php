...
if (!isset($_GET['id_match']))
...
else
 {
  ...
  $requete="SELECT cldom.nom AS cldom, clext.nom AS clext, phpfed2_journees.numero, phpfed2_matchs.date_reelle, phpfed2_matchs.buts_dom, phpfed2_matchs.buts_ext, phpfed2_matchs.date_reelle,
                   cldom.url_logo AS logo_dom, clext.url_logo AS logo_ext
            FROM phpfed2_clubs AS cldom, phpfed2_clubs AS clext, phpfed2_equipes AS dom, phpfed2_equipes AS ext, phpfed2_matchs, phpfed2_journees
            WHERE phpfed2_matchs.id = '$id_match'
  ...
  $requete1="SELECT nom, prenom, phpfed2_joueurs.id as id_joueur
             FROM phpfed2_joueurs, phpfed2_effectif, phpfed2_buteurs, phpfed2_equipes, phpfed2_matchs
             WHERE phpfed2_joueurs.id=phpfed2_effectif.id_joueur
             AND phpfed2_effectif.id=phpfed2_buteurs.id_effectif
             AND phpfed2_buteurs.id_match='$id_match'
             AND phpfed2_matchs.id_equipe_dom = phpfed2_equipes.id
             AND phpfed2_equipes.id=phpfed2_effectif.id_equipe
             AND phpfed2_matchs.id=phpfed2_buteurs.id_match";
  $resultats1=mysql_query($requete1) or die (mysql_error());
  

  $requete2="SELECT nom, prenom, phpfed2_joueurs.id as id_joueur
             FROM phpfed2_joueurs, phpfed2_effectif, phpfed2_buteurs, phpfed2_equipes, phpfed2_matchs
             WHERE phpfed2_joueurs.id=phpfed2_effectif.id_joueur
             AND phpfed2_effectif.id=phpfed2_buteurs.id_effectif
             AND phpfed2_buteurs.id_match='$id_match'
             AND phpfed2_matchs.id_equipe_ext = phpfed2_equipes.id
             AND phpfed2_equipes.id=phpfed2_effectif.id_equipe
             AND phpfed2_matchs.id=phpfed2_buteurs.id_match";
