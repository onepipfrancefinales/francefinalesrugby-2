...
<p class="afficheEquipe"> 
  ...
  <?php
	$reponse = $bdd->query("SELECT bdsaisons.id, bdclubs.nom_1
							...
							AND bdclubs.type = 'M'
							AND (bdsaisons.$terr_annee like '%Top 14%' 
							OR bdsaisons.$terr_annee like '%Pro D2%')");
...
<p class="afficheEquipe">
  ...
  $reponse = $bdd->query ("SELECT bdsaisons.id, bdclubs.nom_1
							...
							AND bdsaisons.siglecomite='$sigle'
							AND bdclubs.type = 'M'
							AND ($terr_annee like '%Federale 1%'  
							OR $terr_annee like '%F&eacute;d&eacute;rale%')");
...
<p class="afficheEquipe"> 
	...
	$reponse = $bdd->query("	
						...
						and bdsaisons.siglecomite='$sigle' 
						AND bdclubs.type = 'M'
						and (bdsaisons.$terr_annee like '%Federale 2%' 
						OR bdsaisons.$terr_annee like'%F&eacute;d&eacute;rale 2%') ");
...
$reponse = $bdd->query("	SELECT bdsaisons.id, bdclubs.nom_1
							...
							and bdsaisons.siglecomite='$sigle' 
							AND bdclubs.type = 'M'
							and ($terr_annee like '%Federale 3%' 
							OR $terr_annee like '%F&eacute;d&eacute;rale 3%')");
...
<p class="afficheEquipe"> 
  <?php
		$reponse = $bdd->query("SELECT bdsaisons.id, bdclubs.nom_1
						...
						AND bdclubs.type = 'M'
						AND bdsaisons.siglecomite='$sigle' 
						AND ($terr_annee like '%Honneur%' 
						OR $terr_annee like '%Promotion%')");
				 while($row = $reponse->fetch())
				 {                          
					$codeEquipe=$row[0];
					echo "<a href=\"pageclub00.php?champion=$codeEquipe\"><img src=\"../images/blasons200_200/$codeEquipe.gif\" height=\"25\" width=\"25\">";
...
<h3>
 ...
 <p class="afficheEquipe">
		...
		$reponse = $bdd->query("SELECT bdsaisons.id, bdclubs.nom_1
						...
						AND bdsaisons.siglecomite='$sigle' 
						AND bdclubs.type = 'M'
						AND ($terr_annee like '1re%' 
						OR $terr_annee like '2me%'  
						OR  $terr_annee like '%1re%')");
...
<p class="afficheEquipe">
	<?php 
		$reponse = $bdd->query("SELECT bdsaisons.id, bdclubs.nom_1
						...
						and bdsaisons.siglecomite='$sigle' 
						AND bdclubs.type = 'M'
						and ($terr_annee like '%3me%' 
						OR $terr_annee like '%4me%')");
