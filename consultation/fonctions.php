...
?>
	...
	FROM bdequipe1
	WHERE champion = '$clubNom' AND (titre = 'Champion' OR titre ='Vice champion') AND categorie ='$categorie'
	");
...
");
	...
	function fusionDeClubsEquipe($nouvelleEntite, $table, $categorie, $bdd)
	{
		global $tabPalmEquipe1, $tabDesClubsFusionnes, $fusion1, $fusion2, $fusion3, $fusion4, $anneeFusion, $nouveauNomClub;
		$res = $bdd->query("SELECT fusion1, fusion2, fusion3, fusion4, annee, nom
			   FROM bdfusion
			   WHERE id = '$nouvelleEntite' 
				");
		...
		while ($row = $res->fetch()) {
			...
			$fusion4 = $row[3];
			$anneeFusion = $row[4];
			$nouvelleEntite = $row[5];
		}
		//TODO  trier le tableau de facon a ne recuperer que les valeur superieure à 0
		$tabDesClubsFusionnes = array();
		for ($i = 1; $i <= 4; $i++) {
			if (${"fusion" . $i} > 0)
				array_push($tabDesClubsFusionnes, ${"fusion" . $i});
			//echo "fusion".$i." : ". ${"fusion".$i} ; echo "<br>";
		}

		for ($p = 1; $p <=  count($tabDesClubsFusionnes); $p++) {
		...
		/************ Equipe 1*************************/
		if ($nouvelleEntite > 0) {
			for ($j = 1; $j <= count($tabDesClubsFusionnes); $j++) {
				...
				$result4 = $bdd->query("SELECT COUNT(*)
										FROM $table
										WHERE champion = '$clubNom' AND (titre = 'Champion' OR titre ='Vice champion'OR titre='Vainqueur' OR titre= 'Finaliste') and categorie = '$categorie'
										");
	...
	function nbreTitre2($nomChampion,  $id_equipe, $table, $categorie, $bdd)
	{
	...
	function affichageCategorie($nomChampion, $clubNom1, $clubNom2, $clubNom3, $clubNom4, $id_equipe, $table, $categorie, $bdd)
	{
		
		
		
		//TODO : recuperer les$clubnom sous la forme d'un tableau
		/*
echo nbreTitre2($nomChampion, $id_equipe, $table, $categorie, $bdd); echo "<br>";
echo nbreTitre2($clubNom1, $id_equipe, $table, $categorie, $bdd);  echo "<br>";
echo nbreTitre2($clubNom2, $id_equipe, $table, $categorie, $bdd);  echo "<br>";
echo nbreTitre2($clubNom3, $id_equipe, $table, $categorie, $bdd);  echo "<br>";
echo nbreTitre2($clubNom4, $id_equipe, $table, $categorie, $bdd);  echo "<br>";
/*
$totaux = nbreTitre2($nomChampion, $id_equipe, $table, $categorie, $bdd) +
			nbreTitre2($clubNom1, $id_equipe, $table, $categorie, $bdd) +
			nbreTitre2($clubNom2, $id_equipe, $table, $categorie, $bdd) +
			nbreTitre2($clubNom3, $id_equipe, $table, $categorie, $bdd) +
			nbreTitre2($clubNom4, $id_equipe, $table, $categorie, $bdd);
*/
		if (substr($nomChampion, 1, 1) == "'")
			$nomChampion = str_replace("'", " ", $nomChampion);
		if (
			nbreTitre2($nomChampion, $id_equipe, $table, $categorie, $bdd) +
			nbreTitre2($clubNom1, $id_equipe, $table, $categorie, $bdd) +
			nbreTitre2($clubNom2, $id_equipe, $table, $categorie, $bdd) +
			nbreTitre2($clubNom3, $id_equipe, $table, $categorie, $bdd) +
			nbreTitre2($clubNom4, $id_equipe, $table, $categorie, $bdd) > 0
		)
		/*
		echo "nomChampion : ".$nomChampion;	echo "<br>";
echo "clubNom1 : ".$clubNom1;	echo "<br>";
echo "clubNom2 : ".$clubNom2;	echo "<br>";
echo "clubNom3 : ".$clubNom3;	echo "<br>";
echo "clubNom4 : ". $clubNom4;	echo "<br>";
